...
const INITIAL_TASKS = [
  {
    ...
    description: "EDA completo del dataset de ventas Q4 con distribuciones y outliers",
    column: "sprint",
    tags: ["eda", "clean"],
    priority: "high",
    sprint: "Sprint 1",
  ...
  },
  {
    ...
    description: "Crear tablero en Power BI con métricas de conversión y retención",
    column: "doing",
    tags: ["viz", "report"],
    priority: "high",
    sprint: "Sprint 1",
  ...
  },
  {
    ...
    description: "Entrenar modelo XGBoost para predicción de abandono de clientes",
    column: "backlog",
    tags: ["ml", "model"],
    priority: "medium",
    sprint: "Sprint 3",
...
function Modal({ task, onSave, onClose }) {
  ...
  const toggleTag = (tid) => {
    set(
      "tags",
      form.tags.includes(tid) ? form.tags.filter((t) => t !== tid) : [...form.tags, tid]
    );
  };
      ...
      >
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
          <div>
            ...
            <select value={form.column} onChange={(e) => set("column", e.target.value)} style={inputStyle}>
              {COLUMNS.map((c) => (
          ...
          <div>
            ...
            <select value={form.points} onChange={(e) => set("points", Number(e.target.value))} style={inputStyle}>
...
export default function App() {
  ...
  // Stats
  const totalPoints = tasks.filter(t => t.sprint === "Sprint 1").reduce((s, t) => s + (t.points || 0), 0);
  const donePoints = tasks.filter(t => t.sprint === "Sprint 1" && t.column === "done").reduce((s, t) => s + (t.points || 0), 0);
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          ...
          <div>
            <div style={{ fontSize: "14px", fontWeight: "700", color: "#e2e8f0", lineHeight: 1 }}>
              DataSprint Board
            </div>
        ...
        <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
          {[
            ...
            >
              <div style={{ fontSize: "14px", fontWeight: "700", color: s.color, fontFamily: "'DM Mono', monospace" }}>
                {s.value}
              </div>
              <div style={{ fontSize: "9px", color: "#4a5568", textTransform: "uppercase", letterSpacing: "0.06em" }}>
                {s.label}
              </div>
            </div>
          ))}
      ...
      >
        ...
        <select
          ...
          style={{ ...inputStyle, width: "130px", background: "#12122a", fontSize: "12px" }}
        >
        ...
        <select
          ...
          style={{ ...inputStyle, width: "140px", background: "#12122a", fontSize: "12px" }}
        >
        ...
        <div style={{ display: "flex", gap: "10px", marginLeft: "auto" }}>
          {PRIORITIES.map((p) => (
            <span key={p.id} style={{ fontSize: "11px", color: "#718096", display: "flex", alignItems: "center", gap: "3px" }}>
            ...
            >
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                  <span style={{ color: col.color, fontSize: "14px" }}>{col.icon}</span>
              ...
              {/* Add from column */}
              <button
                onClick={() => setModal({ mode: "new", task: { column: col.id } })}
                style={{
                  width: "100%",
                  background: "none",
                  border: "1px dashed #2d2d4e",
        ...
        <Modal
          task={
            modal.mode === "edit"
              ? modal.task
              : modal.task
              ? { ...{ title: "", description: "", tags: [], priority: "medium", sprint: "Sprint 1", points: 3, assignee: "Tú" }, ...modal.task }
              : null
          }
          onSave={saveTask}
